...
function setMode(mode) {
  ...
  // Update all elements with data-en / data-kr
  document.querySelectorAll("[data-en]").forEach(el => {
    const val = mode === "kr" ? (el.dataset.kr || el.dataset.en) : el.dataset.en;
    if (val !== undefined) {
      // Use innerHTML for elements that may have & entities
      if (el.tagName === "P" || el.tagName === "SPAN" || el.tagName === "BUTTON" || el.tagName === "H2") {
        el.innerHTML = val;
      } else {
        el.textContent = val;
      }
  ...
  // Reset Q&A thread on language change
  ...
  if (mode === "kr") {
    showBubble("switched to Korean! 한국어 모드 🇰🇷");
  }
}
...
// Nav hover → robot hint
navItems.forEach(item => {
  item.addEventListener("mouseenter", () => {
    const hint = item.dataset.robot;
...
const QA = {
  who: {
    en: 'ChaeYoung "Klara" Kim — compliance auditor by day, robot/IoT security researcher on the side. BoB 13th cohort. Currently at Deloitte Anjin. I built a tool that turns audit findings into attack paths. Feel free to ask more!',
  ...
  deloitte: {
    ...
    kr: "딜로이트 안진 T&T 사이버 리스크 & 컴플라이언스 팀이에요. ISMS-P 갭 분석, 감사 증거 검토, SOC 2 통제 테스트, 제3자 보안 평가가 주요 업무예요. '감사를 통과한 것'과 '실제 공격을 막을 수 있는 것' 사이의 간극을 찾아내는 게 재밌어요."
  },
  comply: {
    en: "Comply-to-Pwn is my research project — a Python CLI (comply2pwn) that takes compliance audit artifacts and maps them to ATT&CK techniques, scores blast radius, then renders an attack graph. Core idea: the paper trail IS the attack surface. If you know what's compliant, you know where the gaps are.",
    kr: "감사 산출물(통제 증거, 갭 파인딩, UAR 결과)을 ATT&CK 기법에 매핑하고, 블래스트 레디우스를 점수화한 뒤 공격 그래프로 시각화하는 Python CLI예요. 핵심 아이디어: '페이퍼트레일 자체가 공격 표면'이에요. 무엇이 통과했는지 알면, 실제 갭도 보입니다."
  },
  robot: {
    en: "Started with LG Electronics — STRIDE-based threat modeling and security requirement derivation for autonomous service robots. Published papers on LINDDUN privacy modeling for robot LTE transmission and STRIDE-based security requirements. Now finalizing R-SPS (a 6-phase security/privacy/safety framework for service robots) for ETRI Journal. Also CubeSat research with Kinryu Labs.",
    kr: "LG전자 서비스 로봇을 시작으로 STRIDE 기반 위협 모델링과 보안 요구사항 도출 연구를 해왔어요. 로봇 LTE 전송 LINDDUN 프라이버시 모델링, STRIDE 기반 보안 요구사항 논문을 발표했고요. 현재 ETRI Journal 투고를 목표로 R-SPS 마무리 중이에요. Kinryu Labs에서 큐브샛 보안 연구도 함께 하고 있어요."
  },
  bob: {
    ...
    kr: "Best of the Best — KITRI 국가 사이버보안 인재 프로그램으로 매년 전국에서 약 30명이 선발돼요. 저는 13기예요. BoB 뉴스레터 6기 기자팀장, 7기 편집장도 맡았어요. 어려운 기술 내용을 현장에 없는 사람에게 설명하는 법을 거기서 배웠는데, 컨설팅의 90%가 그 일이더라고요."
  },
  now: {
    en: "Two things in parallel — finalizing R-SPS for submission to ETRI Journal (a 6-phase security/privacy/safety framework for service robots with a real case study), and ongoing CubeSat security research with Kinryu Labs. Also thinking about the next version of Comply-to-Pwn.",
    kr: "두 가지를 동시에 진행 중이에요. ETRI Journal에 R-SPS 논문 투고 마무리 — 서비스 로봇을 위한 6단계 보안/프라이버시/안전성 평가 프레임워크예요. 그리고 Kinryu Labs에서 큐브샛 보안 연구를 계속하고 있어요. Comply-to-Pwn 다음 버전도 구상 중이고요."
  }
};
...
qaChips.forEach(chip => {
  chip.addEventListener("click", () => {
    ...
    const answer = QA[key] && QA[key][currentMode] ? QA[key][currentMode] : "";
    if (!answer) return;
...
function openTerminal()  { if (terminalOverlay) { terminalOverlay.hidden = false; if (terminalInput) terminalInput.focus(); } }
function closeTerminal() { if (terminalOverlay) terminalOverlay.hidden = true; }
...
if (terminalOverlay)  terminalOverlay.addEventListener("click", e => { if (e.target === terminalOverlay) closeTerminal(); });

document.addEventListener("keydown", e => {
  const typing = document.activeElement && (document.activeElement.tagName === "INPUT" || document.activeElement.tagName === "TEXTAREA");
  if (e.key === "`" && !typing) { e.preventDefault(); if (terminalOverlay && terminalOverlay.hidden) openTerminal(); else closeTerminal(); }
  if (e.key === "Escape" && terminalOverlay && !terminalOverlay.hidden) closeTerminal();
});
...
const CMDS = {
  help:    function() { tPrint("help  whoami  ls  neofetch  lang  cat resume.txt  clear", "t-output"); tPrint("  panels: " + NAV_TARGETS.join(", "), "t-output"); },
  whoami:  function() { tPrint('ChaeYoung "Klara" Kim — Cyber Risk & Compliance / Robot·IoT·CPS Security Researcher', "t-output"); },
  ls:      function() { tPrint(NAV_TARGETS.join("  "), "t-output"); },
  neofetch:function() { ["klara@security","──────────────────","Role:    Cyber Risk & Compliance Consultant","Focus:   ISMS-P · SOC 2 · HIPAA · Robot/IoT/CPS Security","Tools:   STRIDE, LINDDUN, Python, comply2pwn","Origin:  SWLUG 25th → BoB 13th → Deloitte","Lang:    " + currentMode.toUpperCase()].forEach(function(l){ tPrint(l,"t-output"); }); },
  lang:    function() { tPrint("current language: " + currentMode.toUpperCase() + " — click the toggle in the topbar to switch", "t-output"); },
  resume:  function() { tPrint("Compliance auditor by day, robot/IoT security researcher on the side.", "t-output"); },
  sudo:    function() { tPrint("permission denied: nice try.", "t-error"); },
  clear:   function() { terminalBody.innerHTML = ""; }
};
...
function handleCmd(raw) {
  ...
  if (NAV_TARGETS.indexOf(lower) !== -1) { tPrint("opening ~/" + lower + " ...", "t-output"); showPanel(lower); setTimeout(closeTerminal, 380); return; }
...
if (mascot && mascotSvg && bubble) {

  var tx = window.innerWidth - 80, ty = window.innerHeight - 90;
  var cx = tx, cy = ty, lastX = tx, facingRight = false, following = false;
  var bubbleTimer = null, blinkTimer = null;
  ...
  var QUIPS = [
    ...
    "ping -i 0.2 klara.kim",
    "sudo apt install coffee",
    "comply → pwn → repeat",
    "STRIDE all the things",
    "0 days until R-SPS 📄",
    "*beep boop* 🤖"
  ];
  ...
  function scheduleBlink() {
    blinkTimer = setTimeout(function() {
      ...
      setTimeout(function() { mascotSvg.classList.remove("mascot-blink"); scheduleBlink(); }, 140);
  ...
  // Welcome on load
  setTimeout(function() {
    showBubble("hi! i'm klara's robot 🤖 hover the menu — or click me!");
  }, 1200);
  ...
  mascot.addEventListener("click", function() {
    ...
    setTimeout(function() { mascotSvg.style.transform = ""; }, 220);
  });

  // Idle bubble every ~22s
  setInterval(function() {
    if (!bubble.classList.contains("is-visible")) {
      var idle = ["...", "still here 👀", "click me!", "*beep boop*", "scanning…", "// idle", "no bugs here"];
  ...
  document.addEventListener("mousemove", function(e) {
    ...
    mascot._idleTimer = setTimeout(function() { following = false; }, 1800);
  });
